...
if [ ${CUR_TEMP} -ge ${MAX_TEMP} ]; then
	...
	echo "(!) High Temperature: ( ${CUR_TEMP}C >= ${MAX_TEMP}C ), reverting to automatic fan control"
else
	# Set to manual mode
	${IPMI_COMMAND} raw 0x30 0x30 0x01 0x00 > /dev/null 2>&1
	# Set to 1440rpm
	${IPMI_COMMAND} raw 0x30 0x30 0x02 0x01 0x19 > /dev/null 2>&1
	echo "(✓) Temperature OK: ( ${CUR_TEMP}C < ${MAX_TEMP}C ), setting fans to manual fan control (1440rpm)"
fi
